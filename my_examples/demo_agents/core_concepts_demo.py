...
from my_examples.shared.utils import setup_logger, display_agent_response
...
class CalculatorTool(BaseTool):
    ...
    async def _run(self, 
                  expression: Annotated[str, "The mathematical expression to evaluate (e.g., '2 + 2', '5 * 3')"]) -> ToolOutput:
        """Evaluate a mathematical expression."""
        logger.info(f"Calculating: {expression}")
        
        # Parse and evaluate the expression safely
        try:
            # Clean the expression to ensure it only contains safe operations
            cleaned_expr = re.sub(r'[^0-9+\-*/().%\s]', '', expression)
            
            # Evaluate using a safe eval approach
            result = eval(cleaned_expr, {"__builtins__": {}})
            
            return ToolOutput(output=f"Result: {result}")
        except Exception as e:
            return ToolOutput(output=f"Error evaluating expression: {str(e)}")
...
researcher_agent = Agent(
    ...
    model="gpt-4",
    tools=[SearchTool(), RetrieveDocumentTool(), CalculatorTool()],
