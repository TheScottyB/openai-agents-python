...
class AgentLifecycleHooks(AgentHooks):
    ...
    async def on_start(
        ...
        logger.info(
            f"User: {context.context.username} (Premium: {context.context.is_premium})"
        )
...
class CustomerServiceHooks(AgentHooks):
    ...
    async def on_agent_response(
        self, response: Union[str, CustomerResponse], context: CustomerContext
    ) -> None:
        ...
        if isinstance(response, str):
            ...
            if response.requires_escalation:
                logger.warning(
                    f"Customer {context.customer_id} issue requires escalation"
                )
...
def create_customer_service_agent() -> Agent[CustomerContext]:
    ...
    agent = Agent[CustomerContext](
        ...
        instructions=r"""You help customers with their inquiries about our products and services,
